...
export default function Dashboard() {
  ...
  const summary = useMemo(() => {
    if (!records.length) {
      return { totalUnits: null, totalSpent: null, lastDate: null, lastUnits: null };
    }
    const totalUnits = records.reduce((s, r) => s + (r.units || 0), 0);
    const totalSpent = records.reduce((s, r) => s + (r.amount || 0), 0);
    const latest = records[0]; // already sorted newest-first
    return {
      totalUnits,
      totalSpent,
  ...
  const chartData = useMemo(() => {
    ...
    return last.map((r) => ({ date: formatShortDate(r.dateTimeISO), units: r.units || 0 }));
  }, [records]);
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-gray-600">Overview from your processed messages</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-6">
          ...
          <div className="text-3xl font-semibold text-gray-900">
            {summary.totalSpent == null ? "—" : `KES ${summary.totalSpent.toLocaleString()}`}
          </div>
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-6">
          <div className="flex items-center gap-3 mb-3">
            ...
            <span className="text-sm font-medium text-gray-600">Last Purchase</span>
          </div>
          <div className="text-3xl font-semibold text-gray-900">
            {summary.lastDate ? formatShortDate(summary.lastDate) : "—"}
          </div>
          <p className="text-sm text-gray-500 mt-2">
            {summary.lastUnits == null ? "No data yet" : `${summary.lastUnits.toFixed(1)} kWh added`}
          </p>
        </div>
      </div>

      <div className="bg-white border border-gray-200 rounded-lg p-6">
        <h3 className="text-lg font-semibold text-gray-900 mb-6">Units Trend (last 7 purchases)</h3>
          ...
          <div style={{ width: "100%", height: "320px", minHeight: "320px", minWidth: "0" }}>
            <ResponsiveContainer width="100%" height="100%" minWidth={300} minHeight={320}>
              <LineChart data={chartData}>
                ...
                />
                <Line type="monotone" dataKey="units" stroke="#2563eb" strokeWidth={2} dot={{ fill: "#2563eb", r: 4 }} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        )}
